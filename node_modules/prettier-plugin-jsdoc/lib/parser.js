...
const getParser = (originalParse, parserName) => function jsdocParser(text, parsersOrOptions, maybeOptions) {
    ...
    const prettierParse = ((_a = (0, utils_1.findPluginByParser)(parserName, options)) === null || _a === void 0 ? void 0 : _a.parse) || originalParse;
    ...
    const eol = options.endOfLine === "auto" ? (0, utils_1.detectEndOfLine)(text) : options.endOfLine;
    options = { ...options, endOfLine: "lf" };
    ast.comments.forEach((comment) => {
        if (!isBlockComment(comment))
            return;
        const tokenIndex = (0, utils_1.findTokenIndex)(ast.tokens, comment);
        ...
        if (!/^\/\*\*[\s\S]+?\*\/$/.test(commentString))
            return;
        const parsed = (0, comment_parser_1.parse)(commentString, {
            spacing: "preserve",
        })[0];
        comment.value = "";
        if (!parsed) {
        ...
        let tags = parsed.tags
            ...
            if (type) {
                ...
                type = (0, utils_1.convertToModernType)(type);
        ...
        if (options.jsdocSeparateTagGroups) {
            tags = tags.flatMap((tag, index) => {
                ...
                if (prevTag &&
                    ...
                    prevTag.tag !== tags_1.DESCRIPTION &&
                    prevTag.tag !== tags_2.SPACE_TAG_DATA.tag &&
                    tag.tag !== tags_2.SPACE_TAG_DATA.tag &&
                    prevTag.tag !== tag.tag) {
                    return [tags_2.SPACE_TAG_DATA, tag];
                }
                return [tag];
        ...
        tags
            ...
            comment.value += (0, stringify_1.stringify)(tagData, tagIndex, finalTagsArray, { ...options, printWidth: commentContentPrintWidth }, maxTagTitleLength, maxTagTypeLength, maxTagNameLength);
        });
        comment.value = comment.value.trimEnd();
        if (comment.value) {
            comment.value = (0, utils_1.addStarsToTheBeginningOfTheLines)(comment.value, options);
        }
        if (eol === "cr") {
