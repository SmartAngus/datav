...
export function calcTextRect(ctx, pen) {
    ...
    for (var _i = 0, lines_1 = lines; _i < lines_1.length; _i++) {
        var item = lines_1[_i];
        ctx.font = "".concat(pen.fontStyle || 'normal', " normal ").concat(pen.fontWeight || 'normal', " ").concat(pen.fontSize, "px/").concat(pen.lineHeight, " ").concat(pen.fontFamily);
...
export function text(ctx, node) {
    ...
    ctx.font = "".concat(node.fontStyle || 'normal', " normal ").concat(node.fontWeight || 'normal', " ").concat(node.fontSize, "px/").concat(node.lineHeight, " ").concat(node.fontFamily);
    if (node.fontColor) {
